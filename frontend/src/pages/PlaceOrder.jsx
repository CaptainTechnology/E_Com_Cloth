...
const PlaceOrder = () => {
  ...
  const submit_handler = async (event) => {
    ...
    // setOrders(new_order)
    try {
      const respone = await axios.post(backend_url + "/api/order/add", { items: cart, ammount: total + delevery_charge, address: user, status: "Processing", date: date, payment: paymetMethod }, { headers: { token } });
